...
def menu():
    # Check for RPS League file, if DNE, create one.
    if os.path.isfile("RPS_League.txt") == False:
        with open("RPS_League.txt", "w") as file:
            file.write("Official RPS League Records\n")
            file.write("-------------------------------------------------------\n")
            file.write("[]")

    # Code below deals with users and user log files, as well as granting a menu interface.
    if os.path.exists(userName+".txt") == True:

        try:
            with open(userName+".txt", "r") as file:       # Opening user log and reading the stats stored at the top
                #firstLine = print(file.readline())
                lineList = file.readlines()
                file.close()
                timeLine = str(lineList[-2])               # Read the line first as a string...
                timeStamp = str(timeLine[:-8])             # Slice the string to get the desired data.
            ...
            # Above data displayed here by the RPS Agent to the User.
            ...
            print("-------------------------------------------------------")
        except IndexError:      # To catch log files that return errors (usually blank log files)
            print("An error occurred with the log file or it did not exist."
                  " It will be erased and a new one will be created.")
            print("-------------------------------------------------------")
            os.remove(userName+".txt")
            menu()

        # This portion is to display long-term stats for a user such as Sessions, Games, Win-Loss, etc.
        while True:
            try:
                statMenuChoice = int(input("Please choose an option:"
                                       "\n1) View all of your stats to date."
                                       "\n2) Play a game of RPS!"
                                       "\n3) View the current ranked players of the RPS League."
                                       "\n4) Exit the program."
                                       "\n-------------------------------------------------------"))
                ...
                # Stat display section ends, prompts existing user if they want to play or exit.
                elif statMenuChoice == 2:
                    print("-------------------------------------------------------")
                    with open(userName+".txt", "a") as file:
                        file.write("\nSESSION START\n"+str(datetime.datetime.now()))
                        file.write("\n-------------------------------------------------------\n")
                    break
                elif statMenuChoice == 3:
                    print("-------------------------------------------------------")
                    with open("RPS_League.txt", "r") as file:
                        rankLine = file.readlines()
                        rankString = str(rankLine[2])
                        ranks = list(eval(rankString))
                        print("Rank data is formatted as Name/Number of Wins/Number of Losses"
                              "\nPlayers are sorted by rank, with first place at the top.")
                        print("-------------------------------------------------------")
                        for players in ranks:
                            print(players, "Win Ratio:", format(players[1]/(players[1]+players[2]),".2f"))
                        print("-------------------------------------------------------")
                        print("-------------------------------------------------------")

                elif statMenuChoice == 4:
                    exit()
                elif statMenuChoice != 1 or statMenuChoice != 2 or statMenuChoice != 3 or statMenuChoice != 4:
    ...
    # the else statement that activates if a user log file doesn't exist; creates new "user profile."
    else:
    ...
    # The actual game menu (instead of the user data menus) starts here.
    while True:
        try:
            ...
            print("Please input a corresponding number.")
            print(err)
...
def main():
    ...
    def gameSentinel():
        ...
        if newgameSentinel == "Y":
            ...
            # HW6A code pasted here to create new stat files regardless of whether user is first time or not.
            # Utlility program obselete. Only good for erasing files.
...
# END DEFINITION OF FUNCTIONS
    ...
    print("Paper played", stats["P"], "times.")
    print("Scissors played", stats["S"], "times.")
    print("Your calculated win ratio is", format(stats["Game Wins"]/stats["Games"], ".2f"))
    print("-------------------------------------------------------")


    # HW6B portion
    if int(stats["Sessions"]) >= 3:     # Checks for "ranked" players
        leagueWins = int(stats["Game Wins"])
        leagueLosses = int(stats["Game Losses"])
        userLeagueRatio = [userName, leagueWins, leagueLosses]
        with open("RPS_League.txt", "r") as file:
            lines = file.readlines()
            listString = str(lines[2])
            scoreList = list(eval(listString))
            file.close()
        with open("RPS_League.txt", "w") as file:
            scoreList.append(userLeagueRatio)

            # HW6B CODE
            # This sorts by the max value of the "game wins" value, and then removes duplicate entries of the same user.
            from itertools import groupby
            scoreList = [max(g ,key=lambda x:x[1]) for _,g in groupby(sorted(scoreList),lambda x: x[0])]
            scoreList.sort(key=lambda x: x[1], reverse=True)          # Sorted alphabetically
            lines[2] = str(scoreList)
